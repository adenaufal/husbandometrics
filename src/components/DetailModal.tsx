...
const DetailModal: React.FC<DetailModalProps> = ({ character, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl bg-modal-bg rounded-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-fade-in-up border-4 border-white group">
        ...
        <div className="w-full md:w-[40%] relative bg-white p-8 flex flex-col items-center justify-start border-b md:border-b-0 md:border-r-2 border-dashed border-slate-300 z-10">
           {/* Decor */}
           ...
           <div className="relative w-full aspect-square max-w-[280px] mt-8 mb-6 group/image cursor-pointer">
             ...
             <div className="relative w-full h-full rounded-2xl overflow-hidden border-4 border-slate-800 shadow-[8px_8px_0px_rgba(0,0,0,0.1)] bg-white transition-all group-hover/image:-translate-y-1 group-hover/image:shadow-[12px_12px_0px_rgba(0,0,0,0.1)]">
               <img
           ...
           <div className="text-center w-full">
                <h2 className="text-3xl font-black text-slate-800 uppercase font-display leading-none mb-1 tracking-tight">{character.name}</h2>
                <div className="flex items-center justify-center gap-2 mb-6">
                    <span className="text-slate-400 font-bold font-mplus">{character.name_jp}</span>
                    <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                    <span className="text-holo-blue font-bold uppercase">{character.source}</span>
                </div>

                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 w-full hover:border-tech-pink/30 transition-colors">
                    <div className="flex justify-between items-center border-b border-dashed border-slate-200 pb-2 mb-2">
                        ...
                        <span className="text-2xl font-black text-slate-700 font-display">{character.weighted_total}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-slate-400 uppercase">Current Trend</span>
                        <span className={`text-sm font-black px-2 py-0.5 rounded ${
                            character.trend === 'RISING' ? 'bg-green-100 text-green-600' :
                            character.trend === 'FALLING' ? 'bg-red-100 text-red-500' : 'bg-slate-100 text-slate-500'
                        }`}>
                            {character.trend}
                        </span>
                    </div>
        ...
        <div className="w-full md:w-[60%] p-8 bg-[url('https://www.transparenttextures.com/patterns/graphy.png')] bg-fixed">
            <div className="flex items-center gap-2 mb-6">
                <div className="bg-deep-violet p-1.5 rounded-lg shadow-sm">
                    <Activity className="w-5 h-5 text-white" />
                </div>
                <h3 className="text-xl font-black text-slate-800 font-display tracking-tight">PERFORMANCE METRICS</h3>
            </div>
                ...
                <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
                            ...
                            />
                            <Tooltip
                                contentStyle={{ backgroundColor: '#1e293b', border: 'none', color: '#fff', borderRadius: '8px', fontFamily: 'Satoshi', fontWeight: 600 }}
                                itemStyle={{ color: '#fff' }}
                            />
                        </RadarChart>
                    </ResponsiveContainer>
            ...
            <div className="relative group/paper">
                 ...
                 <div className="bg-[#fff9c4] p-6 rounded-sm shadow-md rotate-1 transform transition-all duration-300 group-hover/paper:rotate-0 group-hover/paper:scale-[1.02] relative cursor-help">
                    <div className="flex justify-between items-center mb-3">
                        <h3 className="text-lg font-black text-slate-800 flex items-center gap-2 font-display">
                        ...
                        >
                            {analyzing ? 'WRITING...' : 'UPDATE'}
                        </button>
                    </div>

                    <div className="font-hand text-slate-700 text-lg leading-relaxed">
                        {analysis ? (
                            <p>{analysis}</p>
                        ) : (
                            <p className="opacity-50">Tap update to request new observational data...</p>
                        )}
                    </div>
                 </div>
            </div>
